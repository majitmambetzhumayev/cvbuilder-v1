...
import type { ContactSettings } from '@/sanity/lib/types'
...
export default function ContactSection({ locale, contactSettings }: ContactSectionProps) {
  ...
  const title = tr(contactSettings?.sectionTitle, locale) || t('title')
  const subtitle = tr(contactSettings?.subtitle, locale) || t('subtitle')
  const successMsg = tr(contactSettings?.successMessage, locale) || t('success')
  const errorMsg = tr(contactSettings?.errorMessage, locale) || t('error')
  ...
  return (
    <section id="contact" className="max-w-2xl mx-auto px-6 py-24">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {status === 'success' && <p className="text-forest-400 text-center">{successMsg}</p>}
